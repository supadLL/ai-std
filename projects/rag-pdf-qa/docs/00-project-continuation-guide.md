...
```text
本地 RAG Agent 初版；enterprise-rag-platform 分支已进入企业级改造并完成第 01/02/03/04/05/06/07/08 步
```
...
app/
  ...
  runtime_settings.py  本地运行时 LLM 和 RAG prompt 设置，数据库内 API Key 加密存储
docs/deployment.md     Docker Compose 和局域网部署说明
```
...
Docker Compose 部署 api / db / qdrant / redis
/health 启动健康检查
